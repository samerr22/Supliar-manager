  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (currentUser.isAdmin) {
        navigate('/View'); // Navigate to profile page for admin
      } else {
        navigate('/profile'); // Navigate to home page for non-admin
      }
    
      }
    } catch (error) {
      dispatch(signInFailure(data.message || 'An error occurred while signing in'));
    }
  };
      ...
      <div className="flex p-3 max-w-3xl mx-auto flex-col md:flex-row md:items-center gap-5">
        ...
        <div className="flex-1">
          <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
            ...
            <button
              className=" bg-gray-800 text-white p-3 rounded-lg w-[460px] h-11 hover:opacity-90"
              type="submit"
              disabled={loading}
            >
              {
